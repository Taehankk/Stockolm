...
const StockInfoList = ({ dataProps, onStockClick }: StockInfoListProps) => {
  const [dataList, setDataList] = useState<StockInfo[]>([]);

  const fetchData = async (dataProps: Stock[]) => {
    try {
      const results = await Promise.all(
        dataProps.map((stock) => getStockData(stock.stockCode))
      );

      setDataList(results.map((result) => result));
    } catch (error) {
    ...
    <div>
      ...
      <div>
        <div className="w-[60rem] min-h-[15rem] max-h-[15rem] border-b-black border-b-[0.1rem] overflow-y-auto overflow-x-hidden custom-scrollbar">
          {dataList.length > 0 ? (
            dataList.flat().map((data, index) => (
              dataProps[index] ? (
              <div
                key={index}
                className="flex justify-around items-center w-[60rem] h-[5rem] cursor-pointer"
                onClick={() => onStockClick && onStockClick(dataProps[index])}
              >
                ...
                <span className={`w-1/6 text-center ${getColorClass(data.prdyVrss)}`}>
                  {formatNumberWithCommas(data.prdyVrss)}
                </span>
                <span className={`w-1/6 text-center ${getColorClass(data.prdyVrss)}`}>
                  {formatNumberWithCommas(data.prdyCtrt)}
                </span>
                <span className="w-1/6 text-center">{formatNumberWithCommas(data.acmlVol)}</span>
                <span className="w-1/6 text-center">
                  {formatTransactionAmount(data.acmlTrPbmn)}
                </span>
              </div>) : null
            ))
          ) : (
            <div className="flex min-h-[14rem] justify-center items-center">
              <span className="text-[1.5rem]">등록된 종목이 없습니다.</span>
            </div>
          )}
        </div>
      </div>
